...
function App() {
  ...
  return (
    <>
      <h1 id="heading">TIRED OF BORING LOREM IPSUM?</h1>
      <div id="generator_div">
        <label for="paraNumbers">Paragraphs:</label>
        <input
          id="paraNumbers"
          type="number"
          onChange={(e) => {
        ...
        <button
          onClick={() => {
            if (Number(para) < 0) {
              return alert(
                "Hey user you know that you wrote negative input 😵"
              );
            } else if (Number(para) > 8) {
              return alert(
                " Hey user you are demanding very much paragraph in one go, kindly take little litlle 😀"
              );
            }
            setGenerate(true);
            setParagraphData(paraData.slice(0, Number(para)));
          }}
        >
          GENERATE
        </button>
      ...
      <div id="generated_paragraphs">
      {generate === true
        ? paragraphData.map((ele, ind) => {
